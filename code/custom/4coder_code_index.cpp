...
code_index_shift(Code_Index_Nest_Ptr_Array *array,
    ...
    for (i32 i = 0; i < count; i += 1, nest_ptr += 1){
        ...
        if (nest) { // NOTE(luis) I don't know why but sometimes nest_ptr_array has a null member
            index_shift(&nest->open.min, old_range, new_size);
            index_shift(&nest->open.max, old_range, new_size);
            if (nest->flags & CODE_INDEX_NEST_IS_CLOSED){
        ...
        if (peek != 0 && peek->kind == TokenBaseKind_StatementClose ||
            ...
            if (note && 
                ...
                if (note->nest) {
                    ...
                    if (sub_kind == TokenCppKind_Struct) 
                        note->nest->flags |= CODE_INDEX_NEST_IS_STRUCT;
                    else 
                        note->nest->flags |= CODE_INDEX_NEST_IS_UNION;
                    
                    if (parent) //NOTE will this function handle this null top-level thing?
                        code_index_push_nest(&parent->nest_list, note->nest); 
                    
                    note->nest_range.min = note->nest->open.max;
                    note->nest_range.max = max_i64; //maybe this is a bad idea? 
                    if (note->nest->flags & CODE_INDEX_NEST_IS_CLOSED) {
                        note->nest_range.max = note->nest->close.max;
                    }
                }
            }
            #endif
...
function Code_Index_Note *
cpp_parse_namespace(Code_Index_File *index, Generic_Parse_State *state, Code_Index_Nest *parent) {
    generic_parse_inc(state);
    generic_parse_skip_soft_tokens(index, state);
    ...
    if (token && token->kind == TokenBaseKind_Identifier) {
        ...
        if (peek != 0 && peek->kind == TokenBaseKind_StatementClose ||
            peek->kind == TokenBaseKind_ScopeOpen){
            Code_Index_Note *note = index_new_note(index, state, Ii64(token), CodeIndexNote_Namespace, parent);
            ...
            if (note && peek->kind == TokenBaseKind_ScopeOpen) {
                ...
                if (note->nest) {
                    ...
                    note->nest->flags |= CODE_INDEX_NEST_IS_NAMESPACE;
...
cpp_parse_type_def(Code_Index_File *index, Generic_Parse_State *state, Code_Index_Nest *parent){
    ...
    for (;;){
        ...
        if (token->kind == TokenBaseKind_Identifier){
            ...
            did_advance = true;
            Token *peek = token_it_read(&state->it);
            if (peek && peek->kind == TokenBaseKind_StatementClose ||
...
#if 0 //this version is not enough for us since we try to detect functions inside generic scopes now
function Code_Index_Note *
cpp_parse_function(Code_Index_File *index, Generic_Parse_State *state, Code_Index_Nest *parent) {
    ...
    if (peek != 0 && peek->sub_kind == TokenCppKind_ParenOp){
        ...
        if (at_paren_close){
            ...
            if (peek != 0 &&
                ...
                note = index_new_note(index, state, Ii64(token), CodeIndexNote_Function, parent);
    ...
    if (reset_point) //NOTE(luis) added this
...
function b32 
looks_like_return_base_type(Token *token) {
    if (token->kind == TokenBaseKind_Identifier) return true;
    else if (token->kind == TokenBaseKind_Keyword) {
        
        if (token->sub_kind == TokenCppKind_Void     ||
...
#define defer auto glue(__defer, __LINE__) = Defer_Statement_Dummy_Type{} + [&]()

//NOTE the biggest drawback with doing it this way and allowing it to be called from generic_parse_scope
//is that we have actually no way to differentiate a function decl with a constructor call 
...
cpp_parse_function(Code_Index_File *index, Generic_Parse_State *state, Code_Index_Nest *parent) {
    ...
    while (token && token->kind != TokenBaseKind_EOF) {
        ...
        if (token->kind == TokenBaseKind_Comment) {
            ...
            if (token->sub_kind == TokenCppKind_Star ||
    ...
    if (!token) return 0;
    if (token->kind != TokenBaseKind_Identifier) return 0; //not a function
    ...
    reset_point = token; //reset point right at paren (should this be moved?)
    ...
    for (;;){
        ...
        if (state->in_preprocessor){
            if (!HasFlag(token->flags, TokenBaseFlag_PreprocessorBody) ||
                ...
                result->flags |= CODE_INDEX_NEST_IS_CLOSED;
...
generic_parse_preprocessor(Code_Index_File *index, Generic_Parse_State *state){
    ...
    for (;;) {
        ...
        if (token->kind == TokenBaseKind_Keyword && 
            (token->sub_kind == TokenCppKind_Struct || token->sub_kind == TokenCppKind_Struct)) {
    ...
    for (;;){
        ...
        if (state->do_cpp_parse) {
            if (token->sub_kind == TokenCppKind_Struct ||
                ...
                Code_Index_Note *note = cpp_parse_type_structure(index, state, result, token->sub_kind);
                if (note && note->nest) {
    ...
    for (;;){
        ...
        if (token->kind == TokenBaseKind_Preprocessor){
            ...
            if (token->sub_kind == TokenCppKind_Struct ||
                token->sub_kind == TokenCppKind_Union ||
                token->sub_kind == TokenCppKind_Enum) {
                Code_Index_Note *note = cpp_parse_type_structure(index, state, 0, token->sub_kind);
                if (note && note->nest) {
                    code_index_push_nest(&index->nest_list, note->nest);
                }
            }
            else if (token->sub_kind == TokenCppKind_Namespace) {
                Code_Index_Note *note = cpp_parse_namespace(index, state, 0);
                if (note && note->nest) {
                    code_index_push_nest(&index->nest_list, note->nest);
                }
            }
            ...
            else if (looks_like_return_base_type(token)) {
    ...
    if (nest != 0){
        ...
        case CodeIndexNest_Preprocessor:
        {
            ...
            if (nest->open.min < pos && nest->open.max <= pos &&
                (!(nest->flags & CODE_INDEX_NEST_IS_CLOSED) || pos < nest->close.min)){
