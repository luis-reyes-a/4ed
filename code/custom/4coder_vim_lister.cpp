...
vim_run_lister(Application_Links *app, Lister *lister, bool is_switch_buffer_lister = false){
    ...
    // NOTE this is to make ctrl+tab peek at first file on initial ctrl+tab (you can't do this always because it causes problems with default nagivate for some reason?...
...
function Lister_Result
vim_run_lister_with_refresh_handler(Application_Links *app, Arena *arena, String_Const_u8 query, Lister_Handlers handlers, bool control_quit_aborts = false){
	...
	if(handlers.refresh != 0){
        ...
        //}
                lister.lister.current->vim_max_col_count = 3;
		result = vim_run_lister(app, lister, control_quit_aborts);
	}
	else{
...
vim_get_file_name_from_user(Application_Links *app, Arena *arena, String_Const_u8 query, View_ID view){
    ...
    Lister_Result l_result = vim_run_lister_with_refresh_handler(app, arena, query, handlers, release_control_key_to_quit);
